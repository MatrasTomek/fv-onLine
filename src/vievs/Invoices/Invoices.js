...
const Invoices = () => {
	...
	const getOdrerDataViev = (
		<form className={styles.orderData} onSubmit={handleGetOrderData}>
			...
			/>
			<button className={styles.downloadButton} type="submit" disabled={!testBase ? false : true}>
	...
	const handleSearchInvoice = async (e) => {
		e.preventDefault();
		dispatch(addSpinner());
		const { data } = await request.get(`/invoice/${invoiceNo}`);
		if (data.isSucces === false) {
			dispatch(removeSpinner());
			dispatch(timeoutShowTask(data.info));
		} else {
			dispatch(removeSpinner());
			const data1 = [];
			data1.push(data);
