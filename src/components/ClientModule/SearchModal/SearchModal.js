...
const SearchModal = ({ isModalOpen, setIsModalOpen }) => {
	...
	const handleOnSubmitByName = async (event) => {
		...
		const { data, status } = await clientRequest.get(`/clients/name/${companyName}`);
	...
	return (
		<Modal isModalOpen={isModalOpen} handleOnCloseModal={handleOnCloseModal}>
			<div className={styles.wrapper}>
				...
				<form className={styles.form} method="get" onSubmit={handleOnSubmitByVat}>
					<div className={styles.search}>
						<input onChange={handleOnChangeVat} type="text" value={vatNo} placeholder="podaj nip klienta" />
					</div>
				...
				<form className={styles.form} method="get" onSubmit={handleOnSubmitByName}>
					<div className={styles.search}>
						<input
