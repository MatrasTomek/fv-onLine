...
import { AddClientForm, AddInfoForm, BackButton, Button, ShowInvoice, SearchModal } from "../../../components";
...
const AddInvoice = () => {
	...
	const description = useSelector((store) => store.description.data);
	...
	const clientFromOrder = useSelector((store) => store.dataFromOrder[0].data[1]);
	...
	const onSubmit = (values) => {
		...
		const invoiceObject = {
			...
			additionalDescription: !values.additionalDescription ? "" : values.additionalDescription,
	...
	const dealer = !testBase ? (
		...
		<div className={styles.customer}>
			...
			<AddClientForm isModalOpen={dealerModalOpen} setIsModalOpen={setDealerModalOpen} dealer={addDealerId} />
		</div>
	);

	return (
		<div className={styles.wrapper}>
			<h1>{!isEdit ? "Dodawanie nowej Faktury" : `Edycja faktury ${editData.invoiceNo}`}</h1>
			{dealer}
			<div className={styles.customer}>
				<h4>Nabywca:</h4>
				...
				<div className={styles.buttons}>
					...
					<Button
						name={!clients.length ? "pobierz klienta" : testBase ? "pobierz klienta" : "zmień klienta"}
						onClick={handleSearchModalOpen}
					/>
				</div>
				<AddClientForm isModalOpen={formModalOpen} setIsModalOpen={setFormModalOpen} />
				<SearchModal isModalOpen={searchModalOpen} setIsModalOpen={setSearchModalOpen} />
			</div>
						...
						>
							<div className={styles.dates}>
								...
								<Field
									name="kindOfPayment"
									component="select"
									initialValue={isEdit ? editData.invoice.kindOfPayment : "przelew"}
								>
							...
							<div className={styles.conditions}>
								...
								<Field name="vat" component="select" initialValue={isEdit ? editData.invoice.vat : ""}>
			...
			<AddInfoForm
				...
				dataFrom={dataFrom}
			/>
			<ShowInvoice isModalOpen={checkModalOpen} setIsModalOpen={setCheckModalOpen} />
		</div>
	);
};
